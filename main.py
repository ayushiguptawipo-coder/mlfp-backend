...
# Granular Sector Mapping for Real Institutional Cohorts
GRANULAR_SECTORS = {
    "HEALTHCARE": ["APOLLOHOSP", "SUNPHARMA", "CIPLA", "DRREDDY"],
...
def calculate_live_fundamentals(ticker_symbol, is_us=False):
    """Calculates full fundamental accounting models directly from verified filings."""
    try:
        ...
        if any(k in sector_str or k in ind_str for k in ["BANK", "FINANCIAL", "INSURANCE"]):
            sector = "BFSI"
        ...
        if sector == "BFSI":
            ...
            ebt = safe_df_get(is_, ["Pretax Income"])
        ...
        return {
            ...
            "specific_industry": info.get("industry", "Global Equity"),
        ...
        print(f"Live Fundamentals Calculation Error: {e}")
        return None
...
# =========================================================
# DYNAMIC BEST-IN-CLASS RISK PARITY PORTFOLIO OPTIMIZER
# =========================================================
@app.get("/api/portfolio-basket")
def get_portfolio_basket(anchor_ticker: str = Query(None)):
    """Dynamically identifies 5 top liquid assets across distinct sectors and optimizes weights."""
    candidate_universe = [
        "RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "SUNPHARMA.NS", "HINDUNILVR.NS",
        "BHARTIARTL.NS", "MARUTI.NS", "TITAN.NS", "ICICIBANK.NS", "INFY.NS"
    ]
    
    # If the user is analyzing a specific stock, ensure it's in the candidate pool
    if anchor_ticker:
    ...
    try:
        ...
        # 2. Risk Parity Allocation (Inverse Volatility Weights)
        ...
        for sym in selected_tickers:
            assets.append({
                "ticker": sym,
                "weight": round(float(weights[sym]), 2),
                "volatility": round(float(vols[sym]) * 100, 2)
            })

        corr_matrix = []
        for sym1 in selected_tickers:
            row = [round(float(corr.loc[sym1, sym2]), 2) for sym2 in selected_tickers]
            corr_matrix.append(row)

        # 3. Expected Return Horizon Projection Curve
        ...
        return sanitize_json({
            ...
            "frontier": {
                "days": horizons,
                "projected_returns": projected_curve,
                "annual_expected_ret": round(float(projected_curve[-1]), 2),
                "portfolio_volatility": round(float(weighted_vol * 100), 2)
            }
        })
    except Exception as e:
...
def analyze_stock(ticker: str, instrument_key: str = Query(None), friction: float = Query(0.0015), neutral_band: float = Query(0.05)):
    ...
    # =========================================================
    # ADVANCED SWING OSCILLATOR & 4-DAY SWING PREDICTION MODEL
    # =========================================================
    ...
    # Combines ATR mean reversion speed with current MACD momentum
    swing_daily_drift = (latest_atr_abs * 0.4) if curr_macd > 0 else -(latest_atr_abs * 0.4)
    # Pull towards 20-SMA if overextended (>4%)
    ...
    for day_i in range(1, 5):
        ...
        swing_forecast_days.append({
            "day": f"+{day_i}D",
            "date": str((datetime.now() + timedelta(days=day_i)).strftime('%b %d')),
            "target": round(float(step_p), 2),
            "upper": round(float(upper_swing), 2),
            "lower": round(float(lower_swing), 2)
        })
        last_p = step_p

    # 1-Year Log-Drift Projection
    ...
    if raw_fundamentals:
        ...
        if live_fund:
            ...
            ui_industry = f"Verified Filings: {live_fund.get('specific_industry', 'Global')}"
    ...
    # =========================================================
    # REBUILT: ACCURATE GRANULAR SECTOR PEER MATCHER
    # =========================================================
    peers_list = []
    matched_cohort = []
    
    # 1. Identify which granular cohort this ticker belongs to
    ...
    # 2. If it's a US stock not in predefined list, match with top US tech/consumer peers
    if is_us_stock and not matched_cohort:
        matched_cohort = [t for t in GRANULAR_SECTORS["US_TECH"] if t != clean_sym]

    # 3. Pull actual peers from the database
    ...
    # 4. Fallback fill if cohort is small
    if len(peers_list) < 5:
